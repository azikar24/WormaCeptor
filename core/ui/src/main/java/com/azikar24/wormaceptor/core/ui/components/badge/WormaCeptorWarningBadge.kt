...
fun WormaCeptorWarningBadge(
    ...
    Surface(
        modifier = modifier,
        shape = CircleShape,
        color = MaterialTheme.colorScheme.error.copy(alpha = if (reduceMotion) 1f else alpha),
    ) {
        Icon(
            ...
            modifier = Modifier
                .padding(WormaCeptorTokens.Spacing.xs.scaled())
                .size(WormaCeptorTokens.Spacing.lg.scaled()),
        )
    }
}
