 ...
 */
internal object ImageMetadataExtractorImpl {
    ...
    private fun extractPngMetadata(data: ByteArray): ImageMetadata {
        ...
        return try {
            ...
            ImageMetadata.unknown(data.size.toLong()).copy(format = "PNG")
        }
    }
     ...
     */
    @Suppress("CyclomaticComplexity", "MagicNumber", "NestedBlockDepth", "LoopWithTooManyJumpStatements")
    private fun extractJpegMetadata(data: ByteArray): ImageMetadata {
        ...
        return try {
            ...
            while (offset < data.size - 1) {
                ...
                if (marker in 0xC0..0xCF && marker !in listOf(0xC4, 0xC8, 0xCC)) {
                    if (offset + 7 <= data.size) {
                        ...
                        val height =
                            data[offset + 3].toInt() and 0xFF shl 8 or (data[offset + 4].toInt() and 0xFF)
                        val width =
                            data[offset + 5].toInt() and 0xFF shl 8 or (data[offset + 6].toInt() and 0xFF)
            ...
            ImageMetadata.unknown(data.size.toLong()).copy(format = "JPEG")
        } catch (_: Exception) {
            ImageMetadata.unknown(data.size.toLong()).copy(format = "JPEG")
        }
    }
    ...
    private fun extractGifMetadata(data: ByteArray): ImageMetadata {
        ...
        return try {
            ...
            ImageMetadata.unknown(data.size.toLong()).copy(format = "GIF")
        }
    }
    ...
    private fun extractWebPMetadata(data: ByteArray): ImageMetadata {
        ...
        return try {
            ...
            ImageMetadata.unknown(data.size.toLong()).copy(format = "WebP")
        }
    }
    ...
    private fun extractWebPVP8(data: ByteArray): ImageMetadata {
        ...
        return try {
            ...
            if (data[offset] != 0x9D.toByte() ||
                data[offset + 1] != 0x01.toByte() ||
                data[offset + 2] != 0x2A.toByte()
            ) {
    ...
    private fun extractBmpMetadata(data: ByteArray): ImageMetadata {
        ...
        return try {
            ...
            ImageMetadata.unknown(data.size.toLong()).copy(format = "BMP")
        }
    }
    ...
    private fun extractIcoMetadata(data: ByteArray): ImageMetadata {
        ...
        return try {
            ...
            ImageMetadata.unknown(data.size.toLong()).copy(format = "ICO")
        }
    }
    ...
    private fun extractSvgMetadata(data: ByteArray): ImageMetadata {
        ...
        return try {
            ...
            defaultMetadata.copy(width = width, height = height)
        } catch (_: Exception) {
            defaultMetadata
        }
    }
